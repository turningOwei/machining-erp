...
const MonthlyOutput: React.FC<MonthlyOutputProps> = ({
  ...
  const [includeOutsourcing, setIncludeOutsourcing] = React.useState(true);
  ...
  const fetchMonthlyOutput = React.useCallback(async (targetYear: number, targetDateType: DateType, targetIncludeOutsourcing: boolean) => {
    setLoading(true);
    setError('');
    try {
      const params = new URLSearchParams({
        ...
        includeOutsourcing: String(targetIncludeOutsourcing),
      });
  ...
  React.useEffect(() => {
    fetchMonthlyOutput(year, dateType, includeOutsourcing);
  }, [year, dateType, includeOutsourcing, fetchMonthlyOutput]);

  const maxChartAmount = Math.max(...data.months.map(item => item.output_amount - item.outsourcing_amount), 1);
  const averageMonthCount = year === currentYear ? currentMonth : 12;
  const average = data.total / averageMonthCount;
  const netOutput = data.total - data.outsourcing_amount;
  const bestMonth = data.months.reduce((best, item) => (
    item.output_amount > best.output_amount ? item : best
  ), data.months[0]);
  const maxCustomerAmount = Math.max(...(customerData?.items || []).map(item => item.output_amount - item.outsourcing_amount), 1);
  ...
  const fetchCustomerMonthlyOutput = async (monthItem: MonthlyOutputItem) => {
    ...
    try {
      const params = new URLSearchParams({
        ...
        dateType,
        includeOutsourcing: String(includeOutsourcing),
      });
    ...
    <div className="flex-1 overflow-y-auto space-y-6 py-4 md:py-8 !w-full !max-w-none !m-0 !p-0">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 px-4 md:px-8">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button
            type="button"
            onClick={() => setIncludeOutsourcing(prev => !prev)}
      ...
      <div className="flex flex-wrap gap-4 px-4 md:px-8">
        ...
        <div className="bg-white border border-zinc-200 rounded-lg p-4 shadow-sm flex-[1_1_180px] min-w-[160px] max-w-[240px]">
          <div className="flex items-center justify-between">
            <p className="text-sm text-zinc-500 font-medium">外协共计</p>
            <ReceiptText className="w-5 h-5 text-orange-500" />
          </div>
          <p className="text-xl font-bold mt-3 text-orange-600">{formatCurrency(data.outsourcing_amount)}</p>
        </div>
        <div className="bg-white border border-zinc-200 rounded-lg p-4 shadow-sm flex-[1_1_180px] min-w-[160px] max-w-[240px]">
          <div className="flex items-center justify-between">
            <p className="text-sm text-zinc-500 font-medium">扣外协后产值</p>
            <BarChart3 className="w-5 h-5 text-cyan-500" />
          </div>
          <p className="text-xl font-bold mt-3">{formatCurrency(netOutput)}</p>
        </div>
        <div className="bg-white border border-zinc-200 rounded-lg p-4 shadow-sm flex-[1_1_180px] min-w-[160px] max-w-[240px]">
          <div className="flex items-center justify-between">
      ...
      <div className="px-4 md:px-8">
        <div className="bg-white border border-zinc-200 rounded-lg shadow-sm p-5">
          <div className="flex items-center justify-between mb-5">
            <h3 className="text-lg font-bold">月度扣外协后产值趋势</h3>
          ...
          <div className="h-80 flex items-end gap-2 border-b border-zinc-200 pb-3">
            {data.months.map(item => {
              const chartAmount = item.output_amount - item.outsourcing_amount;
              const height = chartAmount > 0 ? Math.max(4, Math.round((chartAmount / maxChartAmount) * 100)) : 0;
              return (
                <div key={item.month} className="flex-1 min-w-0 h-full flex flex-col justify-end gap-2 group">
                  <div className="text-[10px] text-zinc-500 text-center truncate opacity-0 group-hover:opacity-100 transition-opacity">
                    {formatCurrency(chartAmount)}
                  </div>
                  <div className="h-64 flex items-end">
                    <button
                      type="button"
                      onClick={() => fetchCustomerMonthlyOutput(item)}
                      disabled={chartAmount <= 0 || loading}
                      ...
                      style={{ height: `${height}%` }}
                      title={chartAmount > 0 ? `查看${item.month}客户扣外协后产值统计` : '暂无扣外协后产值'}
                    />
                  </div>
          ...
          <table className="w-full text-sm">
            <thead className="bg-zinc-50 text-zinc-500">
              <tr>
                ...
                <th className="text-right px-4 py-3 font-medium">外协共计</th>
                <th className="text-right px-4 py-3 font-medium">扣外协后产值</th>
            ...
            <tbody>
              {data.months.map(item => (
                <tr key={item.month} className="border-t border-zinc-100">
                  ...
                  <td className="px-4 py-3 text-right text-orange-600">{formatCurrency(item.outsourcing_amount)}</td>
                  <td className="px-4 py-3 text-right font-medium">{formatCurrency(item.output_amount - item.outsourcing_amount)}</td>
        ...
        <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-5xl max-h-[90vh] overflow-hidden flex flex-col">
            <div className="px-6 py-4 border-b border-zinc-100 flex items-center justify-between gap-4">
              <div>
                <h3 className="text-xl font-bold">{selectedMonth.month} 客户扣外协后产值统计</h3>
                <p className="text-sm text-zinc-500 mt-1">
                  按{dateType === 'order_date' ? '订单日期' : '完工日期'}统计，产值 {formatCurrency(customerData?.total || selectedMonth.output_amount)}，外协 {formatCurrency(customerData?.outsourcing_amount || selectedMonth.outsourcing_amount)}，扣外协后产值 {formatCurrency((customerData?.total || selectedMonth.output_amount) - (customerData?.outsourcing_amount || selectedMonth.outsourcing_amount))}
                </p>
              </div>
                  ...
                  {detailChartType === 'bar' ? (
                    <div className="space-y-3">
                      {customerData.items.map((item, index) => {
                        const customerNetOutput = item.output_amount - item.outsourcing_amount;
                        const customerNetTotal = customerData.total - customerData.outsourcing_amount;
                        const width = maxCustomerAmount > 0 ? (Number(customerNetOutput || 0) / maxCustomerAmount) * 100 : 0;
                        const percent = customerNetTotal > 0 ? (customerNetOutput / customerNetTotal) * 100 : 0;
                        return (
                          <div key={item.customer_name} className="grid grid-cols-[160px_1fr_120px] gap-3 items-center">
                            ...
                            <div className="relative h-8">
                              <svg className="block h-8 w-full overflow-visible" viewBox="0 0 100 32" preserveAspectRatio="none" aria-label={`${item.customer_name} 扣外协后产值占最大客户 ${width.toFixed(1)}%`}>
                            ...
                            <div className="text-right text-sm font-bold">{formatCurrency(customerNetOutput)}</div>
                          </div>
                        );
                      })}
                    </div>
                          ...
                          return (
                            <div key={item.customer_name} className="flex items-center gap-3 p-3 border border-zinc-100 rounded-lg">
                              ...
                              <div className="min-w-0 flex-1">
                                ...
                                <div className="text-xs text-zinc-500">{percent.toFixed(1)}% · {formatCurrency(customerNetOutput)}</div>
                              </div>
                            </div>
                          );
                        })}
